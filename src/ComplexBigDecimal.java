...
public class ComplexBigDecimal {
	...
	//									  для ввода коэф	обыч вид или экспонен    конст для latex	  точность вычислений    точность отображения
	public String formattedString(boolean isOutput, boolean isNormalView, boolean isSimpleExponentialView, int accuracyRound, int accuracyView) {
					//обычные значения   true           меняется              true                         меняется в Main    меняется
						
						//при 
						//вводе
						//коэф в решении	 false                                false                        
