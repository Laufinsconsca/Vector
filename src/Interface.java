...
public class Interface extends JFrame {
	...
	public void createGUI() {
		...
		addWindowListener(new WindowListener() {
			...
			public void windowClosing(WindowEvent event) {
				...
				final int n = JOptionPane.showOptionDialog(event.getWindow(), "Закрыть программу?",
						"Подтверждение действия", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE,
		...
		box = new JComboBox<>();
		box.setModel(new DefaultComboBoxModel<Object>(new String[] { "Линейное уравнение", "Квадратное уравнение", "Кубическое уравнение", "Уравнение четвертой степени" }));
		panel.add(box);
		...
		public void actionPerformed(ActionEvent event) {
			...
			if (event.getSource() == aboutButton) {
				JOptionPane.showMessageDialog(null, "Программа предназначена для вычисления корней \n" + "уравнения четвертой степени, \n" 
			        	...
			        	+ "\n" + "Разработчик: Валентин Логачев \n" + "\n                                   2017 год", "О программе", JOptionPane.INFORMATION_MESSAGE);
				focus();
			}
		}
	};
